...
├── litellm/
│   └── observability_callback.py  # LiteLLM 回调：TTFT/生成/错误/token + /metrics
└── blackbox/
    └── blackbox.yml         # http_2xx 探针模块
```

另有对既有部署文件的增改（request_id 贯穿）：
- `execution/proto-remote-access/vps/nginx/private-llm.conf`：注入 `X-Request-Id` + access log 带 rid。
- `execution/proto-remote-access/compat/compat_proxy.py`：新增 `/metrics`、request_id 贯穿、compat 分段耗时。

## 快速启用（部署时执行，见 RUNBOOK.md）

```bash
cd execution/observability
