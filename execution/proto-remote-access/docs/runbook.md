...
10. **用量逐请求明细（2026-08-15，参照 sub2api 日志视图）**：「用量总览」页新增请求明细卡——`GET /console/api/usage/logs`（/spend/logs 本地展开，上限 500 行，鉴权失败脏行沿用 ak_valid 口径过滤）：时间 / Key（别名）/ 模型 / 调用类型 / 输入·缓存读·输出·总计 token / 延迟 / 状态，支持搜索（模型/request_id/别名/错误文本）与状态筛选 + 分页（20/页）；行详情抽屉含 request_id、session_id、失败原因。与既有聚合表、按 Key 占比、近期错误卡并存。
