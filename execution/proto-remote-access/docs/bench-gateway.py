...
KEY = "sk-467AoB6Ihf1RKCm5kiRg2Q"
GW = "https://private-llm.onlyservice.io"
DIRECT = "http://192.168.88.181:8890"
...
def bench(tag, reps):
    ...
    conn = http.client.HTTPSConnection("private-llm.onlyservice.io", context=ctx, timeout=600)
    gws = []
    for _ in range(3):
        t0 = time.time()
...
ctx = ssl.create_default_context()
conn = http.client.HTTPSConnection("private-llm.onlyservice.io", context=ctx, timeout=120)
ts = []
for _ in range(5):
    t0 = time.time()
