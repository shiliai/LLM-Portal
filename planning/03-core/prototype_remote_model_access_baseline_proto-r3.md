# 用户故事基线 proto-r3：远程访问私有推理模型（快速原型）

> 状态：**BASELINE_APPROVED（已冻结）**
> baseline_revision：`proto-r3`（取代 proto-r2，见文末修订说明）
> 权威来源：2026-08-14 会话用户原始需求 + 澄清问答 + 三轮范围扩展指示（多站点/一键隧道/公钥管理；管理页/用量自查；模型映射/网关托管视觉 MCP）
> 批准者：用户（Chris Wang）
> 批准证据：2026-08-14 会话 AskUserQuestion「US-P4 形态」，用户选择「A：纯网关托管」（该问题明确「批准即连同 US-P11 一起冻结 proto-r3」）；r1/r2 批准证据见对应文件
> 范围声明：独立于 LLM-portal 旧 PRD 基线（r2~r4），互不引用、互不约束。快速原型专用。

## 背景事实（用户环境）

- 局域网站点内私有推理服务（无公网 IP，不可被公网直接访问）。首个站点 192.168.88.181：
  - `deepseek-v4-flash-0731`：文本模型，1M 上下文，OpenAI 兼容接口 `http://192.168.88.181:8890/v1`
  - `qwen3.6-35b-a3`：多模态模型，OpenAI 兼容接口 `http://192.168.88.181:8004/v1`
...
### US-P4 网关托管视觉 MCP（r3 修订，形态 A：纯网关托管）
...
- Given 客户端以 `https://<域名>/mcp/vision` + 同一 API Key 注册网关 MCP（Streamable HTTP，注册形态同智谱 vision-mcp-server），When agent 对图片 URL 调用 `analyze_image(url, 问题)`，Then 网关用注册的多模态模型（qwen3.6-35b-a3）识别并返回结果。
...
- 旧 PRD r4 的企业功能（模型映射策略引擎按 Key 生效、system prompt 策略等；注意 US-P11 仅为全局静态别名映射）。
...
| 9 | 视觉 MCP 位置（用户主动）+ toolcall 机制澄清 | 网关托管（Streamable HTTP，同智谱形态）；确认 toolcall 只传路径/URL 后选定形态 A（纯网关托管 + 本地图上传两步） |

**r2→r3 修订说明**：US-P4 由「本地 stdio MCP」改为「网关托管视觉 MCP（形态 A）」，新增上传换临时 URL 的本地图路径、用量归属调用者 Key、无效 Key/无效图片边界；新增 US-P11 模型名映射；non-goals 增补「本地 stdio 视觉桥」并标注 US-P11 为全局静态映射（按 Key 维度策略仍属旧 PRD 企业功能，不在本原型）。US-P1~P3、US-P5~P10、C1~C4 逐字保留自 proto-r2。
