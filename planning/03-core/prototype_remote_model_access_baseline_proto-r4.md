# 用户故事基线 proto-r4：远程访问私有推理模型（快速原型）

> 状态：**BASELINE_APPROVED（已冻结）**
> baseline_revision：`proto-r4`（取代 proto-r3，见文末修订说明）
> 权威来源：2026-08-14 会话用户原始需求 + 澄清问答 + 四轮范围扩展指示（多站点/一键隧道/公钥管理；管理页/用量自查；模型映射/网关托管视觉 MCP；凭据模型/外部 MCP 注册）
> 批准者：用户（Chris Wang）
> 批准证据：2026-08-14 会话 AskUserQuestion「r4 基线批准」，用户选择「批准，冻结 proto-r4」；r1~r3 批准证据见对应文件
...
**r3→r4 修订说明**：US-P4 增补凭据模型条款（用户 Key 永不出网关；无鉴权上游直连；外部服务用网关代持凭据），MCP 端点由 `/mcp/vision` 泛化为 `/mcp`；新增 US-P12 外部 MCP 服务注册；新增 C5。其余故事、约束、non-goals 逐字保留自 proto-r3。
