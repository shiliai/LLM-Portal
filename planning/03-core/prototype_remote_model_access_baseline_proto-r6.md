# 用户故事基线 proto-r6：远程访问私有推理模型（快速原型）

> 状态：**BASELINE_APPROVED（已冻结）**
> baseline_revision：`proto-r6`（取代 proto-r5，见文末修订说明）
> 权威来源：2026-08-14 会话用户原始需求 + 澄清问答 + 六轮范围扩展指示（多站点/一键隧道/公钥管理；管理页/用量自查；模型映射/网关托管视觉 MCP；凭据模型/外部 MCP 注册；Key 绑定 provider 分组；自写管理控制台/第三方管理面收敛）
> 批准者：用户（Chris Wang）
> 批准证据：2026-08-14 会话用户指示「目前这个分支基础上（pr5）修正设计范围，增加自定义的控制台，LiteLLM 对外不暴露，只有管理员可以通过自定义控制台来访问」+ AskUserQuestion「用户自查形态」用户选择「管理员 + 用户自查页」+ 职责边界确认「LiteLLM 退居纯底层模块，引擎级配置留 SSH 低频操作」；r1~r5 批准证据见对应文件
> 范围声明：独立于 LLM-portal 旧 PRD 基线（r2~r4），互不引用、互不约束。快速原型专用。

## 背景事实（用户环境）

- 局域网站点内私有推理服务（无公网 IP，不可被公网直接访问）。首个站点 192.0.2.10：
  - `deepseek-v4-flash-0731`：文本模型，1M 上下文，OpenAI 兼容接口 `http://192.0.2.10:8890/v1`（上游无鉴权）
  - `qwen3.6-35b-a3`：多模态模型，OpenAI 兼容接口 `http://192.0.2.10:8004/v1`（上游无鉴权）
...
- 管理面现状事实（2026-08-14 上线后确认）：MVP 管理面复用 LiteLLM Admin UI（`/ui`），其能力与自写高保真原型（`remote-access-console`，9 页）存在四块缺口——站点/WG 隧道生命周期、分组一等对象化、用户自查页、网关托管 MCP 管理面；此为 r6 增写控制台的直接动因。
...
- Given 客户端以 `https://<域名>/mcp` + 同一 API Key 注册网关 MCP（Streamable HTTP，注册形态同智谱 vision-mcp-server），When agent 对图片 URL 调用 `analyze_image(url, 问题)`，Then 网关用注册的多模态模型（qwen3.6-35b-a3）识别并返回。
...
- （r6 修订）「管理页」指网关**自写 Web 控制台**（`/console/`，Master Key 登录），不再指 LiteLLM 内置 Admin UI；LiteLLM 退居底层执行与记账引擎，其管理面不对公网暴露（详见 US-P14）。

### US-P10 用户用量自查（r6 增补页面形态）
...
- （r6 增补）「页面」形态落地为控制台「我的用量」页：用户以自己的虚拟 Key 在 `/console/` 登录，仅可见该页（API 与 MCP 用量）；「API」形态 `GET /key/info` 与 `GET /mcp/usage` 并存不变。
...
### US-P14 自写管理控制台与暴露面收敛（r6 新增）

As a 管理员 / I want to 用网关自带的 Web 控制台（而非第三方组件的内置界面）完成全部日常管理，且第三方组件的管理面不再对公网暴露 / So that 管理体验与产品形态统一可控、公网攻击面最小。

- Given 管理员在 `https://<域名>/console/` 以 Master Key 登录，When 操作站点接入/吊销、分组管理、模型别名、用户 Key、用量视图、外部 MCP 注册，Then 全部经自写控制台完成并即时生效，无需登服务器或访问第三方 UI。
- Given 部署完成，When 从公网访问 LiteLLM 的 `/ui`、登录与管理类 API 或 onboardd 的 admin API，Then 一律 404；公网仅保留 `/`（主页）、`/v1/*`（双协议 API）、`/key/info`（用户自查）、`/health/liveliness`（状态灯）、`/mcp*`（MCP 端点）、`/onboard/install|register|confirm`（站点接入）与 `/console/`。
- Given 普通用户以自己的虚拟 Key 在控制台登录，When 进入，Then 仅可见「我的用量」（自身 Key 的 API 与 MCP 用量），不可见任何管理页与他人数据。
- 职责边界：运行时数据面（站点/分组/模型别名/Key/用量/MCP）以控制台为唯一管理入口，底层组件（LiteLLM）退居执行与记账引擎；引擎级配置（路由策略参数、分组钩子、版本升级）仍属部署期文件配置（SSH + 部署脚本），不进管理页。
- 静态别名迁移：既有 config.yaml 静态别名一次性迁为控制台可管理的动态条目，模型/别名的唯一事实来源收敛到控制台。
...
- 控制台的多管理员/角色权限体系、操作审计日志（r6 明确排除，单管理员 + C3 口径不变）。
...
| 12 | 范围扩展（用户主动）：增加自定义控制台，LiteLLM 对外不暴露 | 自写控制台为唯一管理入口（master key 登录 8 管理页）；LiteLLM UI/管理 API/onboardd admin 公网 404；用户 Key 可登控制台但仅见「我的用量」；LiteLLM 退居底层模块，引擎级配置留 SSH 低频操作；静态别名迁移为控制台管理 |

**r5→r6 修订说明**：新增 US-P14（自写管理控制台与暴露面收敛：控制台唯一管理入口、公网保留面清单、用户自查页、职责边界、静态别名迁移）；US-P9 修订（管理页=自写控制台，不再指 LiteLLM Admin UI）；US-P10 增补（页面形态=控制台「我的用量」页）；背景事实增补管理面现状（LiteLLM UI 与高保真原型的四块缺口）；non-goals 增补「多管理员/角色权限体系、操作审计」。US-P1~P8、P11~P13、C1~C5 逐字保留自 proto-r5。
